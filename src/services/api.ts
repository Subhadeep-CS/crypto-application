...
export const fetchTrendingCoinData = async () => {
  const response = await fetch(
    `${import.meta.env.VITE_BASE_URL}/search/trending`,
    {
      headers: {
        "Content-Type": "application/json",
        "x-cg-demo-api-key": import.meta.env.VITE_API_KEY,
...
export const fetchAllCoinList = async ({ queryKey }: CoinListQueryKey) => {
  ...
  const response = await fetch(
    ...
    }/coins/markets?vs_currency=${vs_currency}&price_change_percentage=1h,24h,7d,30d&sparkline=true&per_page=${per_page}&page=${page}`,
    {
      headers: {
        "Content-Type": "application/json",
        "x-cg-demo-api-key": import.meta.env.VITE_API_KEY,
...
export const fetchCoinDetails = async ({
  ...
  [string, { coin_id: string | undefined; days: string }]
>) => {
...
export const fetchCoinCategoryList = async () => {
  const response = await fetch(
    `${import.meta.env.VITE_BASE_URL}/coins/categories/list`,
    {
      headers: {
        "Content-Type": "application/json",
        "x-cg-demo-api-key": import.meta.env.VITE_API_KEY,
...
export const fetchCoinCategoryDetails = async () => {
  const response = await fetch(
    `${import.meta.env.VITE_BASE_URL}coins/categories`,
    {
      headers: {
        "Content-Type": "application/json",
        "x-cg-demo-api-key": import.meta.env.VITE_API_KEY,
...
export const fetchSearchData = async (searchQuery: string) => {
  const response = await fetch(
    `${import.meta.env.VITE_BASE_URL}search?query=${searchQuery}`,
    {
      headers: {
        "Content-Type": "application/json",
        "x-cg-demo-api-key": import.meta.env.VITE_API_KEY,
